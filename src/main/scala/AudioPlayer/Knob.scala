...
/* Let the dragging of the rotateable trigger the angle-calculation ------------------------ */
  ...
  onDoubleClick  --> { e => println("leClick "); ; toggleMute}



  //onDragDetected --> { e => println("Detected"); ; initDrag(mousePosition)}
  //onDragOver  	 --> { e => println("Over    "); ; currDragAngle := adjustKnobValue(mousePosition)}
  //onDragDone     --> { e => println("Done    "); ; lastAnglePosition = currDragAngle}
  //onDoubleClick  --> { e => println("leClick "); ; toggleMute}							//Let double-click toggle mute.
...
/* Variables and methods to carry through the calculation of a new angle/value ------------- */
  ...
  private def quadrant(p:Point):Int = {					// Returns the quadrant of a point.
    if(relX(p) >= 0) {if(relY(p) >= 0) 1 else 4}  		// We define the quadrants from 1-4
    else			 {if(relY(p) >= 0) 2 else 3}		// stepping with 90 degrees, starting
  }														// with the range 0 to 90.
  
  private def quadDistance(p:Point) =  					// Returns the number of quadrants 
		  		quadrant(p) - startDragQuadrant + 1		// between start and current 
		  												// drag-position.
  ...
  //override def getChildrenUnprotected = getChildren  	// From the Parent-Trait(abstract)
}
/* === Knob ===================================== END ======================================= */